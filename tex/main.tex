...
\begin{document}
  ...
  A continuación presentamos algunas propiedades adicionales de los grupos de
  Dedekind que resultan fundamentales para comprender su estructura. Estas
  propiedades serán verificadas computacionalmente en la sección~4.

  \begin{proposition}[Nilpotencia]\label{prop:nilpotency}
    Todo grupo de Dedekind es nilpotente. Más aún, los grupos hamiltonianos
    tienen clase de nilpotencia exactamente 2.
  \end{proposition}

  \begin{proposition}[Resolubilidad]\label{prop:solvability}
    Todo grupo de Dedekind es resoluble. Los grupos de Dedekind abelianos
    tienen longitud derivada 1, mientras que los grupos hamiltonianos tienen
    longitud derivada 2.
  \end{proposition}
  ...
  Como consecuencia directa del teorema de caracterización, podemos determinar
  la estructura del subgrupo conmutador y del centro de cualquier grupo
  hamiltoniano:

  \begin{proposition}[Subgrupo conmutador]\label{prop:commutator}
    El subgrupo conmutador de un grupo hamiltoniano es $G' = \{\pm 1\}$, donde
    $\{\pm 1\}$ es el centro de $Q_8$. En particular, $|G'| = 2$.
  \end{proposition}

  \begin{proposition}[Estructura del centro]\label{prop:center}
    El centro de un grupo hamiltoniano de la forma del
    Teorema~\ref{thm:dedekind-baer} es $Z(G) = \{\pm 1\} \times A \times B$,
    donde $\{\pm 1\}$ es el centro de $Q_8$.
  \end{proposition}
  ...
  \begin{listing}[H]
  \inputminted[firstline=29,lastline=37]{julia}{../src/DedekindGroups.jl}
  ...
  \begin{listing}[H]
  \inputminted[firstline=53,lastline=55]{julia}{../src/DedekindGroups.jl}
  ...
  \begin{listing}[H]
  \inputminted[firstline=64,lastline=66]{julia}{../src/DedekindGroups.jl}
  \caption{Construcción del grupo de cuaterniones $Q_8$}
  \end{listing}

  Finalmente, para construir productos directos de grupos (como los del
  Ejemplo~\ref{ex:hamiltonian-finite}), utilizamos la siguiente función:

  \begin{listing}[H]
  \inputminted[firstline=82,lastline=85]{julia}{../src/DedekindGroups.jl}
  ...
  grupos hamiltonianos del Ejemplo~\ref{ex:hamiltonian-finite} en las
  ...
  Dedekind, hamiltoniano), información sobre subgrupos, centro, subgrupo
  conmutador, y las propiedades adicionales estudiadas en la sección anterior
  (clase de nilpotencia, longitud derivada, exponente e índice del centro).
  ...
  no son abelianos. Además, verifican computacionalmente las propiedades
  teóricas enunciadas en las Proposiciones~\ref{prop:nilpotency}--\ref{prop:center}:

  \begin{itemize}
    \item \textbf{Nilpotencia}: Todos los ejemplos tienen clase de nilpotencia
      2, confirmando que los grupos hamiltonianos son nilpotentes de clase
      exactamente 2.

    \item \textbf{Resolubilidad}: Todos los ejemplos tienen longitud derivada
      2, verificando que los grupos hamiltonianos son resolubles pero no
      abelianos.

    \item \textbf{Subgrupo conmutador}: El orden del subgrupo conmutador es
      invariantemente 2 en todos los casos, confirmando la
      Proposición~\ref{prop:commutator}. La estructura del subgrupo conmutador
      es siempre $C_2$ (el grupo cíclico de orden 2), correspondiente a
      $\{\pm 1\}$ en $Q_8$.

    \item \textbf{Centro}: El orden del centro crece según el patrón predicho
      por la Proposición~\ref{prop:center}. En efecto, observamos que
      $|Z(Q_8)| = 2$, $|Z(Q_8 \times \mathbb{Z}/2\mathbb{Z})| = 4$,
      $|Z(Q_8 \times \mathbb{Z}/3\mathbb{Z})| = 6$, y
      $|Z(Q_8 \times (\mathbb{Z}/2\mathbb{Z})^2 \times \mathbb{Z}/3\mathbb{Z})| = 24 = 2 \times 4 \times 3$,
      confirmando que $Z(G) = \{\pm 1\} \times A \times B$.

    \item \textbf{Exponente}: El exponente de cada grupo es el mínimo común
      múltiplo de los exponentes de los factores directos. Por ejemplo,
      $Q_8 \times \mathbb{Z}/3\mathbb{Z}$ tiene exponente 12 (mcm de 4 y 3),
      mientras que $Q_8 \times (\mathbb{Z}/2\mathbb{Z})^2 \times \mathbb{Z}/3\mathbb{Z}$
      también tiene exponente 12 (mcm de 4, 2 y 3).

    \item \textbf{Índice del centro}: El índice $[G:Z(G)]$ proporciona
      información sobre qué tan ``alejado'' está el grupo de ser abeliano.
      Para $Q_8$ el índice es 4, y este valor se mantiene o varía según la
      estructura de los factores adicionales en el producto directo.
  \end{itemize}
